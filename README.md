...
- Диск на бесплатном тарифе не постоянный: `tournament.db` может обнуляться
  ...
  восстанавливает данные за 1-3 минуты без ручных действий. Чтобы данные не
  терялись вообще — см. следующий раздел про Turso.

## Постоянное хранилище (Turso)

По умолчанию — локальный файл `tournament.db`. На Render (эфемерный диск)
это значит, что данные обнуляются при каждом деплое/рестарте, и сайту
приходится собирать турнир заново с нуля. Чтобы данные переживали деплои —
подключите бесплатную БД [Turso](https://turso.tech) (SQLite-совместимая,
работает как отдельный сервис):

1. Зарегистрируйтесь на [turso.tech](https://turso.tech) и создайте базу
   через дашборд (CLI не нужен).
2. В интерфейсе базы найдите **Database URL** (вида
   `libsql://<db>-<org>.turso.io`) и создайте **Auth Token**.
3. Локально: впишите оба значения в `.env`:
   ```
   TURSO_DATABASE_URL=libsql://...
   TURSO_AUTH_TOKEN=...
   ```
4. На Render: добавьте те же две переменные в Environment (дашборд сервиса).

Если эти переменные не заданы — всё работает как раньше, на локальном
файле. Никакой миграции/переключения кода не требуется, только переменные
окружения.
